...
export default function FieldMappingRow({ entity, source, field }: FieldMappingRowProps) {
  const { getMappingForField, openMappingModal, deleteMapping, isFieldFavourite, toggleFieldFavourite } = useHarmonizationStore();

  const mapping = getMappingForField(entity.id, source.id, field.id);
  const isMapped = !!mapping;
  const isFavourite = isFieldFavourite(entity.id, source.id, field.id);
  ...
  return (
    <tr className={`${isMapped ? 'bg-green-50/50' : 'hover:bg-gray-50'}`}>
      ...
      <td className="px-2 py-3 text-center w-10">
        <button
          onClick={() => toggleFieldFavourite(entity.id, source.id, field.id)}
