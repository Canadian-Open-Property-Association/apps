...
function SortablePropertyNode({ property, depth }: PropertyNodeProps) {
  const {
    selectedPropertyId,
    expandedNodes,
    selectProperty,
    ...
    <div ref={setNodeRef} style={style}>
      ...
      >
        ...
        <button
          {...attributes}
          {...listeners}
          className="p-0.5 hover:bg-gray-200 rounded cursor-grab active:cursor-grabbing touch-none"
      ...
      {isExpanded && hasChildren && (
        <div>
          {property.type === 'object' && property.properties && property.properties.length > 0 && (
            ...
            >
              <SortableContext
                items={property.properties.map(p => p.id)}
                strategy={verticalListSortingStrategy}
              >
                {property.properties.map((child) => (
                  <SortablePropertyNode key={child.id} property={child} depth={depth + 1} />
                ))}
              </SortableContext>
            </DndContext>
          )}
          ...
          >
            ...
            >
              {properties.map((property) => (
                <SortablePropertyNode key={property.id} property={property} depth={0} />
              ))}
            </SortableContext>
          </DndContext>
        )}
