...
function getSampleValue(property: Property): unknown {
  if (property.sampleValue) {
    ...
    // Return as-is for strings or numbers
  ...
  const placeholders: Record<PropertyValueType, unknown> = {
    string: '<string>',
    number: 0,
    boolean: false,
    date: '2024-01-15',
...
export default function JsonPreviewModal({ property, dataTypeName, onClose }: JsonPreviewModalProps) {
  const sampleValue = getSampleValue(property);

  // Build the credential payload structure
  const credentialPayload = {
    '@context': [
      'https://www.w3.org/2018/credentials/v1',
      'https://copa.ca/credentials/v1',
    ],
    type: ['VerifiableCredential', dataTypeName.replace(/\s+/g, '')],
    credentialSubject: {
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <dl className="grid grid-cols-2 gap-3 text-sm">
              ...
              {property.providerMappings && property.providerMappings.length > 0 && (
                <div className="col-span-2">
                  <dt className="text-gray-500 mb-1">Provider Mappings</dt>
                  <dd className="space-y-1">
                    {property.providerMappings.map((mapping) => (
                      <div key={mapping.entityId} className="text-xs bg-white rounded px-2 py-1 border border-gray-200">
                        <span className="font-medium">{mapping.entityName || mapping.entityId}</span>
                        <span className="text-gray-400 mx-1">&rarr;</span>
                        <span className="font-mono text-blue-600">{mapping.providerFieldName}</span>
                      </div>
                    ))}
                  </dd>
                </div>
